...
const connection = mysql.createConnection(
  ...
  exports.viewAllEmployees = () => {
    ...
    "e2.first_name, e2.last_name, CONCAT(e2.first_name, ' ', e2.last_name) AS manager " +
    "FROM employee AS E " +
    "INNER JOIN employee_role AS C ON E.emp_role_id = c.role_id " +
    "INNER JOIN department AS D ON C.dept_id = d.dept_id " +
    "LEFT JOIN employee AS E2 ON E.manager_id = E2.emp_id ORDER BY emp_id;";
...
exports.viewAllDepartments = () => {
    const depSearch = "SELECT * FROM department ORDER BY dept_id";
...
exports.getAllRoles = (cb) => {
    connection.query("SELECT * FROM employee_role ORDER BY role_id;", function(err,results) {
        if(err) {throw err};
        cb(results);
    });
};
